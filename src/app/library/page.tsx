...
import { FaPlus, FaPencilAlt, FaTrashAlt, FaSearch, FaEye } from 'react-icons/fa'; 
    ...
    return (
        <div className="max-w-7xl mx-auto p-6">
            <div className="flex justify-between items-center mb-6">
                ...
                {/* Adicionar Livro */}
                <Button asChild><Link href="/add-book" className="flex items-center"><span><FaPlus className="mr-2" /> Adicionar Livro</span></Link></Button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredBooks.length === 0 ? (
                    ...
                    filteredBooks.map((book: Book) => ( 
                        ...
                        <div key={book.id} className="bg-card text-card-foreground rounded-lg shadow-lg border border-border overflow-hidden flex flex-col h-full">
                            ...
                            <div className="relative w-full h-80 group">
                                ...
                                <div className="absolute inset-0 bg-background/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                                    <div className="flex gap-4">
                                        ...
                                        <Button asChild className="p-2 h-10 w-10 rounded-full cursor-pointer bg-blue-500/80 hover:bg-blue-600"><Link href={`/book/${book.id}`} className="flex items-center justify-center"><FaEye className="text-white" /></Link></Button>

                                        {/* Botão Editar */}
                                        <Button asChild className="p-2 h-10 w-10 rounded-full cursor-pointer bg-yellow-500/80 hover:bg-yellow-600"><Link href={`/edit-book/${book.id}`} className="flex items-center justify-center"><FaPencilAlt className="text-white" /></Link></Button>
                                        
                                        {/* Botão Excluir (Trigger) */}
                                        <AlertDialog>
                                            <AlertDialogTrigger asChild><Button variant="destructive" className="p-2 h-10 w-10 rounded-full cursor-pointer bg-red-600/80 hover:bg-red-700"><FaTrashAlt className="text-white" /></Button></AlertDialogTrigger>
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                ...
                                <div className="p-2 flex flex-col items-center text-center cursor-pointer h-full">
                                    ...
                                    <div className="flex flex-col items-center w-full flex-shrink-0">
                                        ...
                                        <div className="flex flex-wrap justify-center items-center gap-1 mt-0">
                                            ...
                                            {/* GÊNERO como Badge */}
                                            {book.genre && (
                                                <Badge variant="secondary" className="text-xs font-medium max-w-full truncate mb-3">
                                                    {book.genre}
                                                </Badge>
                                            )}
                                            ...
                                            {book.status && (
                                                <Badge variant={getStatusBadgeVariant(book.status)} className="text-xs font-medium mb-3">
